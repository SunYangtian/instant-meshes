...
std::vector<Eigen::MatrixXd>  instant_mesh_interface(MatrixXf& V, MatrixXu& F,
    ...
    // MatrixXf V = V_;
    // MatrixXu F = F_;
    // MatrixXu F = MatrixXu::Random(3, 30);
    VectorXf A;
    ...
    cout << "vertex shape: " << O_extr.size() << endl;
    cout << "face shape: " << F_extr.size() << endl;
    if (bvh)
        delete bvh;

    std::vector<Eigen::MatrixXd> output_bundle = {O_extr.cast<double>(), F_extr.cast<double>()};
    return output_bundle;
}


PYBIND11_MODULE(InstantMeshes, m) {
    m.doc() = "pybind11 InstantMeshes plugin"; // optional module docstring

    m.def("instant_mesh_interface", &instant_mesh_interface, "A function which runs instant mesh");
}
...
//     MatrixXf V, N;
//     MatrixXu F;
//     std::string input = "/mnt/sda/syt/code/mesh_process/debug.obj";
...
//     instant_mesh_interface(V, F, rosy, posy, scale, face_count, vertex_count, creaseAngle, extrinsic, align_to_boundaries, smooth_iter, knn_points, pure_quad, deterministic);
// }